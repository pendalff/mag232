...
class Widget extends Template{
	...
	public function getInstagramDataRequest() {
	...
	public function getWidgetInstagramData() {
		...
		if($instagramToken!=''){
			...
			$table      = $resource->getTableName('mgs_instagram_cache');
			$select     = $connection->select()->from($table)->where('store_id = ?', $storeId);
			$row        = $connection->fetchRow($select);
			
			if (empty($row)) {
				
				$result = $this->getInstagramDataRequest();
				
				if($result){
					
					$dataInstagram = serialize($result);
					
					$instagramData['instagram_data'] = $dataInstagram;
					$instagramData['store_id'] = $storeId;
					$instagramData['creation_time'] = $this->_date->gmtDate('Y-m-d H:i:s');
					$connection->insert($table, $instagramData);
					
				}
			} else {
				$timestamp   = strtotime($row['creation_time']);
				$currentDate = strtotime($this->_date->gmtDate('Y-m-d H:i:s'));
				
				$timeLast = round($currentDate - $timestamp) / 60;

				if($timeLast <= 15){
					$insData = $row['instagram_data'];
					$result = unserialize($insData);
				}else {
